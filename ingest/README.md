...
- **The PDF text-extraction backend defaults to `pypdfium`** (`AGRIAI_DOCLING_BACKEND`: `pypdfium` | `default` | `parse-v4`), chosen by an A/B run, not by preference. Against Docling's own backend it cut truncated words from 5 to 2, doubled complete occurrences of "biometric" and "Plant height", grew the tomato article from 85 to 98 chunks, removed the passage where ANOVA numbers were spliced into body prose, and recovered the crop calendar's July column (the default backend read it as `inr`/`nf`/`ot`). Its cost: it garbles characters on 4 chunks, which the screener below removes. We take that trade because the default backend's damage is *silent* — spliced table numbers still read like prose — while pypdfium's is visible and detectable. Re-measure with:

  ```bash
  python run.py --dry-run --dump                                 # pypdfium (default)
  AGRIAI_DOCLING_BACKEND=default python run.py --dry-run --dump  # Docling's own
  ```

- **Chunks are screened before they reach the corpus** (`screen_chunks` in `run.py`), for two reasons: *too short* (< 15 tokens — page furniture like "Article") and *garbled extraction* (glued case like `ltithEG`, or vowel-free lowercase blobs like `ttkftd`). Measured over 277 chunks across both documents and both backends: 0 false positives, and it catches exactly the 4 damaged pypdfium chunks. Every dropped chunk is printed with its reason and written to the dump.
